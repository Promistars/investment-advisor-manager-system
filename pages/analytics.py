...
if not is_client_mode:
    ...
    with col_input:
        ...
        if st.session_state.violation_msg:
            ...
            with st.container():
                ...
                if st.button("✅ 确认并录入指令", width="stretch", type="primary"):
    ...
    with st.expander("🛠️ 强制补录历史结账 (防止收益回撤错失结算点)", expanded=False):
        ...
        if not hist_admin.empty:
            ...
            with mb1:
                if st.button(f"💸 按 {manual_date} 补录内扣", width="stretch", key="m_in"):
                    add_and_save_billing(manual_date, '提取管理费(内扣)', target_hist_watermark - manual_fee, manual_fee)
            with mb2:
                if st.button(f"🤝 按 {manual_date} 补录外付", width="stretch", key="m_out"):
            ...
            for r_name, txt in saved_comms.items():
                ...
                with st.expander(f"📄 {active_acc} - {r_name}"):
                    ...
                    if st.session_state.get(confirm_key, False):
                        ...
                        if c_yes.button("🚨 我已知晓，彻底删除", key=f"do_{confirm_key}", type="primary", width="stretch"):
                            db.delete_commentary(current_user, active_acc, r_name)
                            st.session_state[confirm_key] = False
                            st.rerun()
                        if c_no.button("取消", key=f"cancel_{confirm_key}", width="stretch"):
                            st.session_state[confirm_key] = False
                            st.rerun()
